...
function renderSidebar(collections) {
  ...
  sidebar.innerHTML = `<h2 class="text-xl font-semibold mb-4">Categorías</h2>` +
    ...
    `</div>`;
  // Mueve el mensaje inferior fuera del innerHTML principal y colócalo en un div aparte
  let footer = document.getElementById('sidebar-footer');
  if (!footer) {
    footer = document.createElement('div');
    footer.id = 'sidebar-footer';
    footer.className = 'mt-auto text-xs text-gray-400 pt-8';
    sidebar.appendChild(footer);
  }
  footer.innerHTML = `<div>Ultima actualización: 03-03-1965</div><div>Creado por juanfranbrv</div>`;
  ...
  grid.innerHTML = filtered.map(bm => {
    ...
    return `
      <div class="bg-white rounded-lg shadow p-4 flex flex-col gap-2">
        <div class="aspect-[4/3] w-full overflow-hidden rounded mb-2">
