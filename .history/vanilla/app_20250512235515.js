...
function renderSidebar(collections) {
  ...
  sidebar.innerHTML = `<h2 class="text-xl font-semibold mb-4">Categorías</h2>` +
    ...
    `</div>`;
  sidebar.innerHTML += `
    <div class="absolute bottom-4 left-4 right-4 text-xs text-gray-400 select-none pointer-events-none">
      <div>Ultima actualización: 12-05-2025</div>
      <div>Creado por juanfranbrv</div>
    </div>
  `;
...
function renderBookmarks(bookmarks, collections, bookmark_tags, tags) {
  ...
  grid.innerHTML = filtered.map(bm => {
    ...
    return `
      <a href="${bm.url}" target="_blank" class="block group focus:outline-none h-full">
        <div class="bg-white rounded-lg shadow flex flex-col h-full min-h-[430px] max-h-[430px] p-0 transition ring-2 ring-transparent group-hover:ring-blue-200 group-focus:ring-blue-400">
          ...
          <div class="p-4 flex flex-col gap-2 flex-1">
            ...
            <div class="text-lg font-semibold text-blue-700 group-hover:underline">${bm.title}</div>
            <p class="text-sm text-gray-600 overflow-hidden" style="display: -webkit-box; -webkit-line-clamp: 3; -webkit-box-orient: vertical; line-clamp: 3;">${bm.comment}</p>
